...
A nonlinear divisor can be contracted by the polar map while (detoperatorname{Hess}F
otequiv0).
...
These calculations are not currently committed as reproducible scripts. If this research is to be audited, the next documentation improvement should be to reconstruct and commit the exact scripts used for:
...
[
	ext{degree-}le5	ext{ scalar map on }mathbb F_p^5
]
...
> On a terminal quartic exceptional divisor, resolve the polar pencil. A generic fibre (A) should satisfy an adjunction/discrepancy condition. Combine this with (edeg Ale16) to limit (A) to low-degree del Pezzo/scroll-type surfaces. Planes, quadrics and cubic scrolls were suspected to be excluded because a covering curve would make (K_A+H|_A) negative.
